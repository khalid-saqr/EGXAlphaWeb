...
export function methodologyPage() {
  return `<style id="methodology-document-styles">${METHODOLOGY_CSS}</style><main class="site-shell page-methodology" data-page="methodology">
    ...
    <article class="research-paper paper-document">
      <header class="research-cover paper-cover"><p class="eyebrow">PUBLIC RESEARCH NOTE</p><h1>EGX /Alpha Methodology</h1><p class="lede">A ranking-first deep-learning research system for Egyptian listed equities, described at the level needed to understand its scientific and operational discipline without disclosing proprietary model implementation.</p><div class="doc-control">${control('MARKET','EGYPTIAN EXCHANGE')}${control('PRIMARY HORIZON','5 SESSIONS')}${control('OUTPUT','CROSS-SECTIONAL RANK')}${control('USE','RESEARCH ONLY')}</div></header>
      <section class="research-abstract paper-abstract"><p class="eyebrow">ABSTRACT</p><p>EGX /Alpha converts time-locked post-close market observations into a relative ranking of the eligible domestic equity universe. The production system validates source freshness, applies a reviewed deep-learning inference package, ranks eligible securities for forward research horizons, stores dated model memory and publishes a sanitized public research record. Exact features, trained parameters, private scores and internal diagnostics remain outside the public boundary.</p></section>
      ...
      <div class="research-body">
        ${section('01','Research objective: relative opportunity', ['The central problem is cross-sectional. On each eligible trading date the system compares Egyptian listed securities with one another and estimates their relative forward position. The public rank should therefore be read as a position within that day’s eligible comparison set, not as a guaranteed return or valuation target.','The primary public research horizon is five EGX trading sessions. Rank and directional interpretation are distinct outputs and should be read together.'])}
        ...
        ${section('06','Public publication boundary', ['The public layer is a one-way disclosure boundary. It is designed to publish the sanitized primary-horizon universe ranking together with date, rank, direction, source state, provenance and research disclaimer while withholding private numerical scores and implementation details.','Historical records remain date-addressable so the system can be inspected longitudinally without rewriting past model outputs.'])}
        ${section('07','Interpretation limits', ['EGX /Alpha is a research-ranking engine operating under changing market regimes and source availability. A rank is conditional on the eligible universe and information available at the cutoff. It is not personalised investment advice, a target price or an execution instruction.'])}
      </div>
      <section class="paper-boundary"><p class="eyebrow">DISCLOSURE BOUNDARY</p><h2>What is public and what remains private</h2><div class="boundary-grid"><article><strong>Public research surface</strong><p>Completed analysis date, eligible-universe context, primary-horizon rank, public direction category, source/publication state, archive provenance and methodology.</p></article><article><strong>Private research implementation</strong><p>Raw source records, exact feature construction, model dimensions, objective coefficients, trained weights, private numerical scores, internal diagnostics and operational paths.</p></article></div></section>
    </article>${megaFooter()}
  </main>`;
}
