...
function hero(payload, previousPayload, canonicalPath) {
  ...
  const liveDeck = hasOneDay ? "Use 1D to prioritise tomorrow's research and 3D to see whether the view persists. Start with the highest percentile names, then read the direction label: a high rank means stronger relative positioning, while Positive / Neutral / Negative is the model's separate directional view. Δ only shows movement versus the previous model run—none of these measures is a price target or probability." : "Read the board from the top: higher percentile means stronger relative positioning inside the eligible market. Direction is the model's separate view, while Δ only shows rank movement versus the previous completed run.";
