...
for (const page of [home,today]) {
  ...
  for (const required of ['EGX Research','EGX /ALPHA · RESEARCH DIGITAL TWIN','See the Egyptian equity market through deep learning.','continuously updated research digital twin of the Egyptian equity market',"TODAY'S DIGITAL-TWIN STATE",'TOP 5 · SELECTED HORIZON','RELATIVE RANK','MODEL DIRECTION','RANK MOVE','MARKET STRUCTURE','MODEL-STATE CHANGES','FULL DIGITAL-TWIN STATE','The complete eligible universe, ordered by the model.','POSITIVE','NEUTRAL','NEGATIVE','LEARN THE MODEL STATE','WHEN MODEL STATES BECOME EVIDENCE','PUBLIC MODEL MEMORY','EGX Research Community LLP','https://knowdyn.com','https://60arabia.com','footer-disclaimer','site-footer','SEARCH','INSTITUTIONAL','aria-current="page"','brand-lockup','alpha-mark','research-footer','nav-desktop','mobile-menu','primary-action hero-primary-action']) assert.ok(html.includes(required),`product UI should include ${required}`);
  for (const removed of ['Deep-learning ranking of Egyptian equities.','After each completed EGX session, /Alpha ranks the eligible stock universe across multiple forward horizons.','RANK PERCENTILE · ZOOMED 90–100','Bar length is public rank percentile','NEXT MODEL OUTLOOK','WHAT /ALPHA SEES NEXT','Highest-ranked public forecasts','hero-forward-1','hero-forward-3','data-hero-forecast-panel','data-hero-chart','Loading historical rank percentile','DEEP-LEARNING MARKET RANKING','OBSERVE','VALIDATE','INFER','>PUBLISH<','CONSTRUCTIVE','CAUTION','mega-footer']) assert.equal(html.includes(removed),false,`old or misleading homepage construct must be removed: ${removed}`);
  if (production.schema_version==='egx_alpha_public_wire_v1') assert.ok(html.includes('PUBLICATION NOTE'));
  else if (production.schema_version==='egx_alpha_public_wire_v2') for(const required of ['RESEARCH HORIZON','>1D<','>3D<','>5D<','>10D<','D = trading days','data-horizon-select="1"','data-horizon-select="10"','PRIMARY RESEARCH VIEW','data-horizon-panel="1"','data-horizon-panel="3"','data-horizon-panel="5"','data-horizon-panel="10"']) assert.ok(html.includes(required),`V2 product UI should include ${required}`);
...
for(const required of ['footer-close','A living research record of the Egyptian equity market.','Relative Rank, Model Direction, Rank Move and related outputs are general model-generated research classifications.','footer-grid','EXPLORE /ALPHA','RESEARCH METHOD','ACCESS','footer-provenance','MACHINE-READABLE RECORD']) assert.ok(footerMarkup.includes(required),`research-close footer should include ${required}`);
...
for(const required of ['.alpha-mark','.brand-lockup','.nav-desktop','.mobile-menu','@media (max-width:900px)','@media (max-width:620px)','@media (prefers-reduced-motion:reduce)','.research-topbar>.brand-lockup{grid-column:1;grid-row:1}','.research-topbar>.nav-desktop{grid-column:2;grid-row:1;width:auto;margin:0}','.research-topbar>.header-actions{grid-column:3;grid-row:1}','.research-topbar>.header-actions{grid-column:2;grid-row:1}']) assert.ok(shellCss.includes(required),`research shell CSS should include ${required}`);
...
assert.ok(terminalCss.includes('.control-deck-copy>.hero-primary-action{margin-top:clamp(10px,1.4vw,18px);justify-content:center;text-align:center;min-height:46px;padding-inline:20px;line-height:1.3}'),'homepage hero CTA must preserve deliberate separation and centered justification');
assert.ok(terminalCss.includes('.control-deck-copy>.hero-primary-action{width:100%;min-height:48px;margin-top:14px;padding-inline:16px}'),'homepage hero CTA must expand cleanly on narrow screens');
