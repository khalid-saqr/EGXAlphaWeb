...
for (const html of [home, today]) {
  ...
  assert.ok(html.includes('investor-signal-card'), 'signal should use the investor decision card');
  assert.ok(html.includes('investor-decision-grid'), 'signal should expose rank, horizon and publication in one reading frame');
  ...
  assert.ok(html.includes('value-proof-grid'), 'hero should explain the system value without repeating signal facts');
  assert.equal(html.includes('mind-summary'), false, 'old duplicate ticker/signal/window summary must be removed');
  assert.equal(html.includes('compact-strip'), false, 'archive/search/methodology navigation must not be duplicated below the hero');
