...
for (const page of [home,today]) {
  ...
  for (const required of ['EGX Research','EGX /ALPHA PUBLIC RESEARCH TERMINAL','How does EGX /Alpha rank the eligible EGX universe for the sessions ahead?','After each completed EGX session, EGX /Alpha publishes a cross-sectional research ranking','general model outputs intended for research and informational use','HIGHEST RELATIVE MODEL RANKS','TOP 5 · SELECTED HORIZON','RELATIVE RANK','MODEL DIRECTION','RANK MOVE','MARKET PULSE','RANK MOVERS','/ALPHA MARKET RANKING','The complete eligible EGX universe.','POSITIVE','NEUTRAL','NEGATIVE','HOW TO READ /ALPHA','RESEARCH EVIDENCE','PUBLIC MODEL MEMORY','EGX Research Community LLP','https://knowdyn.com','https://60arabia.com','footer-disclaimer','site-footer','SEARCH','INSTITUTIONAL','aria-current="page"','brand-lockup','alpha-mark','research-footer','nav-desktop','mobile-menu']) assert.ok(html.includes(required),`product UI should include ${required}`);
...
for(const required of ['footer-close','A daily cross-sectional research ranking of the Egyptian market.','Relative Rank and Model Direction provide general quantitative research observations for independent interpretation.','footer-grid','USE /ALPHA','UNDERSTAND /ALPHA','ACCESS','footer-provenance','VERIFY PUBLIC RECORD']) assert.ok(footerMarkup.includes(required),`research-close footer should include ${required}`);
