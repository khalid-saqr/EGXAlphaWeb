...
for (const page of [home,today]) {
  ...
  for (const required of ['NEXT MODEL OUTLOOK',"Tomorrow's strongest model forecasts.",'WHAT /ALPHA SEES NEXT','Highest-ranked public forecasts','RANK PERCENTILE · ZOOMED 90–100','NEXT TRADING DAY','THREE TRADING DAYS','Δ','RANK PERCENTILE',"TODAY'S MARKET RANKING",'POSITIVE','NEUTRAL','NEGATIVE','HOW TO READ /ALPHA','MODEL EVIDENCE','site-footer','SEARCH','INSTITUTIONAL','aria-current="page"']) assert.ok(html.includes(required),`product UI should include ${required}`);
  for (const removed of ['See how the model ranks the Egyptian market.','data-hero-chart','Loading historical rank percentile','DEEP-LEARNING MARKET RANKING','Cross-sectional ranking','OBSERVE','VALIDATE','INFER','PUBLISH','CONSTRUCTIVE','CAUTION','mega-footer']) assert.equal(html.includes(removed),false,`stale project/history-first component or copy must be removed: ${removed}`);
